...
/*
  ...
  only the element parameter is required though
*/

// CODE HERE
const evenNumbers = mixedNumbers.filter((elem) => {
  return (elem%2 === 0)
})
...
/*
  ...
  Use a 7% tax rate.
  Recall that .map returns a copy of the original array, with any changes implemented in the callback applied
...
// CODE HERE
const postTaxPrices = prices.map(elem => (elem*(1 + 0.07)).toFixed(2)); //toFixed() converts the # to a string and 
console.log(postTaxPrices);                                             //rounds to string to the specfied # of decimals
...
const totalAfterTax = orders.map( elem => (elem.price*(elem.tax + 1)).toFixed(2))
...
const bobsTotal = purchases.filter(elem => elem.owner === "Bob").reduce((acc,curr) => acc += curr.price, 0);
//We have to set the initial value of reduce function to 0, because the initial value (the default) is not 0/number; 
//it is an object. 
